...
class Prediction_Data_validation:
    ...
    def __init__(self,path):
        self.Batch_Directory = path # Directory where the prediction data files are stored
        self.schema_path = 'schema_prediction.json' # path where the schema is stored
    ...
    def valuesFromSchema(self):
        ...
        try:
            with open(self.schema_path, 'r') as f:
                dic = json.load(f)
                f.close()
            pattern = dic['SampleFileName'] # Unused variable....
    ...
    def deleteExistingGoodDataTrainingFolder(self):
        ...
        """
        try:
            path = 'Prediction_Raw_Files_Validated/' # Directory to store the Good data files
    ...
    def deletePredictionFile(self):
        ...
        Description: This function deletes the existing prediction file from the directory
        Output: None
        On Failure: Exception
        ...
        if os.path.exists('Prediction_Output_File/Predictions.csv'): # if file exists
            os.remove('Prediction_Output_File/Predictions.csv') # delete the file
            ...
            for file in listdir('Prediction_Raw_Files_Validated/Good_Raw/'):
                ...
                for columns in csv:
                    if (len(csv[columns]) - csv[columns].count()) == len(csv[columns]):
                        ...
                        break
                if count==0:
                    csv.rename(columns={"Unnamed: 0": "forest_cover"}, inplace=True)
                    csv.to_csv("Prediction_Raw_Files_Validated/Good_Raw/" + file, index=None, header=True)
        except OSError:
            f = open("Prediction_Logs/missingValuesInColumn.txt", 'a+')
            self.logger.log(f, "Error Occured while moving the file :: %s" % OSError)
