...
class Preprocessor:
    ...
    def scaleData(self,data):
        ...
        Description: This method scales the data using StandardScaler.
        Output: A scaled version of the DataFrame which has been passed to this method
        On Failure: Raise Exception
        ...
        num_data = data[
            ["elevation", "aspect", "slope", "horizontal_distance_to_hydrology", "Vertical_Distance_To_Hydrology",
             "Horizontal_Distance_To_Roadways", "Horizontal_Distance_To_Fire_Points"]] # select only the numerical data
        cat_data = data.drop(
            ["elevation", "aspect", "slope", "horizontal_distance_to_hydrology", "Vertical_Distance_To_Hydrology",
        ...
        # concatenate the scaled data with the categorical data
        final_data = pd.concat([num_data, cat_data], axis=1)

        return final_data # return the scaled dataframe

    def enocdeCategoricalvalues(self,data):
        """
        Method Name: encodeCategoricalvalues
        Description: This method encodes the categorical data.
        Output: Dtaframe with encoded categorical data
        On Failure: Raise Exception
        ...
        try:
            ...
            data["class"] = data["class"].map(
                ...
                "Cottonwood_Willow": 6})
            self.logger_object.log(self.file_object, 'Encoding categorical values Successful. Exited the encodeCategoricalvalues method of the Preprocessor class')

            return data
        except Exception as exe:
            self.logger_object.log(self.file_object,'Exception occured in encodeCategoricalvalues method of the Preprocessor class. Exception message:  ' + str(exe))
            self.logger_object.log(self.file_object, 'Encoding categorical values Failed. Exited the encodeCategoricalvalues method of the Preprocessor class')
            raise exe
    ...
    def handleImbalanceDataset(self,X,y):
        ...
        Description: This method handles the imbalance in the dataset using SMOTE.
        Output: X, and y with balanced data.
        On Failure: Raise Exception
        ...
        """
        try:
            self.logger_object.log(self.file_object, 'Entered the handleImbalanceDataset method of the Preprocessor class')
            # Create an object of the SMOTE class
            sample = SMOTE()
            # Fit the data to the object
            X, y = sample.fit_resample(X, y)
            self.logger_object.log(self.file_object, 'Handling Imbalance Dataset Successful. Exited the handleImbalanceDataset method of the Preprocessor class')
            return X,y # return the balanced data

        except Exception as exe:
            self.logger_object.log(self.file_object,'Exception occured in handleImbalanceDataset method of the Preprocessor class. Exception message:  ' + str(exe))
            self.logger_object.log(self.file_object, 'handleImbalanceDataset Failed. Exited the handleImbalanceDataset method of the Preprocessor class')
            raise exe